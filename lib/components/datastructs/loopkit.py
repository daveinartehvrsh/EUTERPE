...
class Loopkit(Container):
    ...
    def fill(self, path, n_loops, info):
        ...
        for i in range(int(n_loops)):
            ...
            logger.info(f'      Trimming {loop.get_name()} at load')
    ...
    def normalize_loop_length(self, info):
        ...
        if 'bar_lenght' not in info:
            ...
            logger.info(f'GLOBAL BAR LENGHT set to {info["bar_lenght"]} samples')

        logger.info(f'      Normalizing loop lenght to {info["bar_lenght"]} samples')
        for item in self.get_items():
